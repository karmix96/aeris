...
app = typer.Typer(help="Aerodynamic analysis commands.")


def _print_aero_result(result) -> None:
...
def _load_aero_sweep_manifest(run_dir: Path) -> dict[str, Any]:
    ...
    # Case 1: user passed the run root
    candidates.append(run_dir / "aero_sweep_manifest.json")

    # Case 2: user passed the run root, nested manifest under aero_sweep/
    candidates.append(run_dir / "aero_sweep" / "aero_sweep_manifest.json")

    # Case 3: user passed the nested aero_sweep/ directory itself
    ...
    for path in candidates:
        if path.exists():
            ...
            # If this is the top-level run manifest, extract the sweep payload.
            if "aero_sweep_result" in data:
                return data

            # If this is the pure sweep manifest written by sweep_run.py,
            # wrap it into the same shape expected by inspect_aero_sweep().
...
def run_aero(
    ...
    mode_count = sum(
        x is not None and x != ""
        for x in [config, run_dir, dataset]
    )
    if mode_count != 1:
    ...
    run_name = output_name.strip() or f"{timestamp}_aero_{label}"
    run_root = (Path.cwd() / "data" / "runs" / run_name).resolve()
    ...
    if config is not None:
        ...
        if config.exists():
            shutil.copy2(config, run_root / config.name)
    ...
    (run_root / "aero_manifest.json").write_text(json.dumps(manifest, indent=2))
...
def sweep_aero(
    ...
    run_name = output_name.strip() or f"{timestamp}_aero_sweep_{label}"
    run_root = (Path.cwd() / "data" / "runs" / run_name).resolve()
    geometry_dir = run_root / "geometry"
    sweep_dir = run_root / "aero_sweep"
    ...
    sweep = FlightConditionSweep(
        alpha_deg_values=_parse_float_list(alpha_values),
        beta_deg_values=_parse_float_list(beta_values),
        velocity_mps_values=_parse_float_list(velocity_values),
        altitude_m_values=_parse_float_list(altitude_values),
        p_rad_s_values=_parse_float_list(p_values),
        q_rad_s_values=_parse_float_list(q_values),
        r_rad_s_values=_parse_float_list(r_values),
    )
    ...
    (run_root / "aero_sweep_manifest.json").write_text(json.dumps(manifest, indent=2))

    typer.echo("")
    typer.echo(f"[AERIS] Aero sweep completed in: {run_root}")
    typer.echo(f"[AERIS] Requested cases: {sweep_result.summary.get('requested_n_cases')}")
    typer.echo(f"[AERIS] Completed cases: {sweep_result.summary.get('completed_n_cases')}")

    success_n = sum(1 for case in sweep_result.cases if case.get("status") == "success")
    typer.echo(f"[AERIS] Successful cases: {success_n}")
    typer.echo(f"[AERIS] Non-success cases: {len(sweep_result.cases) - success_n}")

    typer.echo("[AERIS] Case statuses:")
    for case in sweep_result.cases:
        typer.echo(f"  - {case['case_label']}: {case['status']}")

@app.command("sweep-inspect")
...
@app.command("sweep-case-inspect")
def inspect_aero_sweep_case(
    ...
    as_json: bool = typer.Option(
        False,
        "--json",
        help="Print the saved aero case result as machine-readable JSON.",
    ),
) -> None:
