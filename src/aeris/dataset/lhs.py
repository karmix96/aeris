...
def _lhs_unit(n_samples: int, n_dim: int, rng: np.random.Generator) -> np.ndarray:
    """
    Generate an n_samples x n_dim Latin hypercube in [0, 1].
    """
    if n_samples < 1:
...
def build_lhs_design_matrix(
    ...
    """
    Returns a numeric matrix of shape (n_samples, 17) in the exact order of
    config.design_variable_names().
    """
    ...
    bounds = [
        ...
        # store internal convention directly: negative aft sweep
...
def lhs_matrix_to_samples(matrix: np.ndarray) -> list[BWBDesignSample]:
    """
    Convert an (n, 17) matrix into a list of BWBDesignSample objects.
    """
...
def generate_lhs_samples(
    ...
    config -> LHS matrix -> list[BWBDesignSample]
    """
