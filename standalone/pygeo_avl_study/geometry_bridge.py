"""Geometry bridge for the standalone pyGeo/AVL feasibility study.

This module deliberately lives outside ``src/aeris``.  It reads the existing
Aeris section records and CST implementation, but it does not register a new
generator or modify any production code.

The central operation is:

    authored stations -> pyGeo B-spline loft -> realised 3-D slices
    -> normalised 2-D section -> CST fit -> AeroSandbox/AVL geometry
...
def pygeo_frame_inputs(
    ...
    if mode != "asb_frame":
        raise ValueError(
            "frame_mode must be one of: asb_frame, global_y, dihedral_euler"
        )
...
def build_pygeo(
    ...
    *,
    ...
    rot_x, rot_y, rot_z, thickness_scale, frame_error = pygeo_frame_inputs(
        stations, frame_mode
    )
    capture = io.StringIO()
...
def extract_section(
    ...
    *,
    ...
    cst, cst_rms = CSTAirfoil.fit(
        x_upper, z_upper, x_lower, z_lower,
        order=int(cst_order), dz_te=dz_te,
    ...
    return ExtractedSection(
        index=int(index), span_fraction=float(span_fraction), v_parameter=v,
        le_xyz_m=np.asarray(le_xyz), te_xyz_m=np.asarray(te_xyz), chord_m=chord_m,
        twist_deg=_signed_twist_deg(chord_axis, span_axis), local_span_axis=span_axis,
        chord_axis=chord_axis, thickness_axis=thickness_axis,
        upper_xyz_m=np.asarray(upper_xyz), lower_xyz_m=np.asarray(lower_xyz),
        x_upper=x_upper, z_upper=z_upper, x_lower=x_lower, z_lower=z_lower,
        direct_coordinates=direct_coordinates, cst=cst, cst_order=int(cst_order),
        cst_rms_chord=float(cst_rms), cst_max_chord=cst_max,
        plane_warp_rms_chord=plane_warp_rms, plane_warp_max_chord=plane_warp_max,
        thickness_ratio=float(np.max(thickness)), max_camber_ratio=float(np.max(np.abs(camber))),
        cst_valid=bool(validation.valid), cst_failures=tuple(validation.failures),
    )
...
def extract_sections(
    ...
    *,
    ...
    return [
        ...
        for i, (frac, v) in enumerate(zip(fractions, parameters))
    ]
