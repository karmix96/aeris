...
def test_dataset_compute_control_derivatives_help_runs() -> None:
    result = runner.invoke(app, ["dataset", "compute-control-derivatives", "--help"])
    assert result.exit_code == 0
    assert "--dataset" in result.stdout
    assert "--min-abs-cm-delta-e" in result.stdout
    assert "symmetric" in result.stdout.lower()


def test_dataset_compute_control_derivatives_cli_writes_outputs(tmp_path: Path) -> None:
    ...
    assert "Control derivative extraction completed" in result.stdout
...
def test_dataset_compute_control_derivatives_cli_json(tmp_path: Path) -> None:
    root = tmp_path / "aero_dataset"
    _make_dataset(root)
    ...
    assert payload["computed_group_count"] == 1
