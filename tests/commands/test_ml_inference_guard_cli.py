...
def _train_and_promote(tmp_path: Path) -> tuple[Path, Path]:
    ...
    promote = runner.invoke(app, [
        "ml", "promote-model",
        "--model-run-dir", str(model_run_dir),
...
def test_ml_check_inference_inputs_cli_runs(tmp_path: Path) -> None:
    ...
    assert "ML inference-input guard" in result.stdout
    assert "passed: True" in result.stdout
    assert (out_dir / "inference_guard_report.json").exists()


def test_ml_predict_cli_enforce_envelope_fails_for_ood_input(tmp_path: Path) -> None:
    _, model_run_dir = _train_and_promote(tmp_path)
