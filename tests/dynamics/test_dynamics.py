...
# ---------------------------------------------------------------------------
# Inertia estimator (datcom.py)
# ---------------------------------------------------------------------------

def test_inertia_estimate_basic():
    """Known BWB: 12.5 kg, 2.0 m span, 0.877 m MAC, 35° sweep."""
    from aeris.dynamics.datcom import estimate_inertia_from_geometry
    est = estimate_inertia_from_geometry(
        mass_kg=12.5,
        span_m=2.0,
        mac_m=0.877,
        area_m2=0.5,
    ...
    # Ixx = 12.5 * (k_x * 1.0)^2  with k_x ~ 0.32 → ~ 1.28 kg·m²
    assert est.ixx_kg_m2 is not None
    assert 0.5 < est.ixx_kg_m2 < 3.0, f"Ixx out of range: {est.ixx_kg_m2}"
    # Iyy = 12.5 * (k_y * 0.877)^2 with k_y ~ 0.37 → ~ 0.56 kg·m²
    assert est.iyy_kg_m2 is not None
    assert 0.2 < est.iyy_kg_m2 < 2.0, f"Iyy out of range: {est.iyy_kg_m2}"
    # Izz = Ixx + Iyy (thin plate)
    assert est.izz_kg_m2 is not None
    assert abs(est.izz_kg_m2 - (est.ixx_kg_m2 + est.iyy_kg_m2)) < 1e-9
    # Ixz small
    assert abs(est.ixz_kg_m2) < est.ixx_kg_m2 * 0.2


def test_inertia_scales_with_mass():
    """Doubling mass should double all inertias (linear)."""
    from aeris.dynamics.datcom import estimate_inertia_from_geometry
    e1 = estimate_inertia_from_geometry(
        mass_kg=10.0, span_m=2.0, mac_m=0.877, area_m2=0.5
    )
    e2 = estimate_inertia_from_geometry(
        mass_kg=20.0, span_m=2.0, mac_m=0.877, area_m2=0.5
    )
    assert abs(e2.ixx_kg_m2 / e1.ixx_kg_m2 - 2.0) < 1e-6
    assert abs(e2.iyy_kg_m2 / e1.iyy_kg_m2 - 2.0) < 1e-6


def test_inertia_sweep_effect():
    """Higher sweep should increase k_y and therefore Iyy."""
    from aeris.dynamics.datcom import estimate_inertia_from_geometry
    e_low  = estimate_inertia_from_geometry(
        mass_kg=12.5, span_m=2.0, mac_m=0.877, area_m2=0.5, sweep_le_deg=20.0
    )
    e_high = estimate_inertia_from_geometry(
        mass_kg=12.5, span_m=2.0, mac_m=0.877, area_m2=0.5, sweep_le_deg=50.0
    )
    assert e_high.iyy_kg_m2 > e_low.iyy_kg_m2, (
        "Higher sweep should increase Iyy (mass further from pitch axis)"
    )


def test_inertia_from_geometry_summary():
    """Test loading from a geometry_summary-like dict."""
    from aeris.dynamics.datcom import inertia_from_geometry_summary
    gs = {
        "reference_values": {
            "span_m": 2.0,
            "area_m2": 0.5,
            "mean_aerodynamic_chord_m": 0.877,
            "aspect_ratio": 8.0,
        },
        "mean_angles_deg": {
            "sweep_le_deg": 35.0,
    ...
    est = inertia_from_geometry_summary(gs, mass_kg=12.5)
    assert est.iyy_kg_m2 is not None and est.iyy_kg_m2 > 0


def test_inertia_to_mass_properties():
    """Round-trip: inertia estimate → MassProperties."""
    from aeris.dynamics.datcom import estimate_inertia_from_geometry, inertia_to_mass_properties
    est = estimate_inertia_from_geometry(
        mass_kg=12.5, span_m=2.0, mac_m=0.877, area_m2=0.5
    )
    mp = inertia_to_mass_properties(est, mass_kg=12.5, x_cg_m=0.40)
    assert mp.mass_kg == 12.5
    assert mp.x_cg_m == 0.40
    assert mp.inertia.iyy_kg_m2 == est.iyy_kg_m2
    assert mp.inertia_complete() is True


# ---------------------------------------------------------------------------
# UQ propagation (uq.py) — unit tests without full AERIS import
# ---------------------------------------------------------------------------

def test_uq_mode_result_basic():
    from aeris.dynamics.uq import _mode_uq_result
    values = [0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3]
    result = _mode_uq_result(
        metric_name="zeta_sp",
        values=values,
        n_total=10,
        point_estimate=0.7,
        level1_check_fn=lambda z: 0.35 <= z <= 1.30,
        level2_check_fn=lambda z: 0.25 <= z <= 2.00,
        stable_check_fn=lambda z: z > 0,
    )
    assert result.n_samples == 10
    assert result.p_level1 == 1.0   # all in [0.35, 1.30]
    assert result.p_level2_or_better == 1.0
    assert result.p_stable == 1.0
...
def test_uq_mode_result_mixed_level():
    from aeris.dynamics.uq import _mode_uq_result
    # Half outside Level 1 band
    values = [0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60, 0.80, 1.20, 1.40]
    result = _mode_uq_result(
        metric_name="zeta_sp",
        values=values,
        n_total=10,
    ...
    # Values in L1 band: 0.35, 0.40, 0.50, 0.60, 0.80, 1.20 = 6/10
    assert abs(result.p_level1 - 0.6) < 1e-9


def test_uq_percentile():
    from aeris.dynamics.uq import _percentile
...
def test_uq_sampling_respects_seed():
    from aeris.dynamics.uq import _sample_uniform_marginal
    import random
    pe = {"CLa": 4.0, "Cma": -2.5}
    qi = {"CLa": 0.3, "Cma": 0.2}
    rng1 = random.Random(42)
    rng2 = random.Random(42)
    s1 = _sample_uniform_marginal(pe, qi, rng1)
    s2 = _sample_uniform_marginal(pe, qi, rng2)
    assert s1 == s2, "Same seed must produce same sample"


def test_uq_sampling_stays_in_interval():
    from aeris.dynamics.uq import _sample_uniform_marginal
    import random
    pe = {"CLa": 4.0, "Cma": -2.5}
    qi = {"CLa": 0.3, "Cma": 0.2}
    ...
    for _ in range(100):
        s = _sample_uniform_marginal(pe, qi, rng)
        assert 3.7 <= s["CLa"] <= 4.3
        assert -2.7 <= s["Cma"] <= -2.3


def test_uq_zero_interval_returns_point():
    """Zero interval width → always returns point estimate."""
    from aeris.dynamics.uq import _sample_uniform_marginal
    import random
    pe = {"CLa": 4.0}
    qi = {"CLa": 0.0}
