...
def _make_geometry_dataset(dataset_root: Path, *, rows: list[dict], succeeded_n: int | None = None) -> None:
    ...
    (dataset_root / "configs" / "input_config.yaml").write_text("name: strict_fail_fixture\n", encoding="utf-8")
...
def test_promotion_strict_rejects_non_monotonic_taper_dataset(tmp_path: Path) -> None:
    dataset_root = tmp_path / "bad_geometry_dataset"

    row = _healthy_geometry_row(dataset_root)
    row["c2_ratio"] = 0.50
    row["c3_ratio"] = 0.70  # strict-only failure: non-monotonic taper
    ...
    assert "geometry_chord_ratio_sanity_v1" in strict_validator_ids